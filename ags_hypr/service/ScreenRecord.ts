...
const ScreenRecorderService = GObject.registerClass(
  ...
  class Recorder extends GObject.Object {
    ...
    setRecordInternalAudio(i: boolean) {
      this.#recordInternalAudio = i;
    }

    setRecordAudio(i: boolean) {
      this.#recordAudio = i;
    }
    ...
    async start() {
      ...
      print(this.#recordAudio);
      const cmd = `wl-screenrec --output ${output} --filename ${this.#file} ${this.#recordAudio ? "--audio" : ""}`;
